...
def generate_rectangles(n):
    ...
    for i in range(1,2*n,2):
        rectangle = plt.Rectangle((x[i-1],0), x[i+1]-x[i-1], y[i], alpha=0.5, color = 'blue') 
...
def low_sum(n):
    ...
    for i in range(n):
        rectangle = plt.Rectangle((x[i],0), (x[i+1]-x[i]), y[i], alpha=0.5, color = 'blue')
        rectangles.append(rectangle)
    return rectangles
...
def upper_sum(n):
    ...
    rectangles = []
    for i in range(n):
        rectangle = plt.Rectangle((x[i],0), (x[i+1]-x[i]), y[i+1], alpha=0.5, color = 'blue')
        rectangles.append(rectangle)
    return rectangles
...
def refresh_animate(i):
    ...
    (rectangles,A)=generate_rectangles(i)
